        ...
        <div class="form-group">
            <div class="form-row">
                ...
                <div class="col-sm-4 mb-2">
                    ...
                    <span class="error-msg" id="error-invoice_number" style="color: red"></span>
                </div>
                <div class="col-sm-4 mb-2">
...
@push('ajax')
    <script>
        $('document').ready(function() {
            ...
            function loadInvoices(selectedId = '') {
                loadershow();
                $('#invoice_number').html(`<option value="">Select Invoice</option>`);

                ajaxRequest('GET', "{{ route('invoice.inv_list') }}", {
                    token: API_TOKEN,
                    company_id: COMPANY_ID,
                    user_id: USER_ID
                }).done(function (response) {
                    if (response.status == 200 && response.data != '') {
                        $.each(response.data, function (key, value) {
                            const invoiceLabel = [value.inv_no, value.inv_date, value.customer]
                                .filter(Boolean).join(' - ');
                            $('#invoice_number').append(
                                `<option value="${value.inv_no}" data-invnumber="${value.inv_no}">${invoiceLabel}</option>`
                            );
                        });
                    } else {
                        $('#invoice_number').append(`<option disabled>No Invoice Found</option>`);
                    }
                      $('#invoice_number').val(selectedId);
                      console.log(selectedId);
                    // ✅ Set selected invoice from edit data
                    if ($('#invoice_number').data('select2')) $('#invoice_number').select2('destroy');
                        $('#invoice_number').val(selectedId).select2({
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            ...
                            console.log(transporterbill.invoice_no);
                            loadInvoices(transporterbill.invoice_no ?? '');
                        } else if (response.status == 500) {
                            Toast.fire({
                                icon: "error",
                                title: response.message
