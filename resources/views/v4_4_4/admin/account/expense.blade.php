...
@section('sidebar-filters')
    <div class="col-12 p-0">
        <div class="card">
            ...
            <div class="card-body">
                <select class="filter select2 form-control w-100" name="filter_paid_to" id="filter_paid_to">
...
@push('ajax')
    <script>
        $('document').ready(function() {
            ...
            function getPadiToData() {
                return new Promise((resolve, reject) => {
                    $.ajax({
                        type: 'GET',
                        url: "{{ route('customer.accountexpensecustomer') }}",
                        data: {
            ...
            function loadFilters() {
                return new Promise((resolve, reject) => {
                    ...
                    if (filterData) {
                        ...
                        $('#filter_paid_to', '#filter_category', '#filter_subcategory')
                            .trigger('change');
            ...
            async function initialize() {
                try {
                    ...
                    const [padiToDataResponse, categoryDataResponse] = await Promise.all([
                        getPadiToData(),
                        getCategoryData()
                    ]);
                    ...
                    if (padiToDataResponse.status == 200 && padiToDataResponse.customer != '') {
                        // You can update your HTML with the data here if needed     
                        $.each(padiToDataResponse.customer, function(key, value) {
                            var optionValue = value.firstname + (value.lastname ? ' ' + value.lastname :
                                '');
                            var id = value.id;
                            $('#filter_paid_to').append(
                                `<option value="${id}">${optionValue}</option>`);
                        });

                        $('#filter_paid_to').val('');
                        $('#filter_paid_to').select2({
                            search: true,
                            placeholder: 'Select Paid To',
                            allowClear: true // Optional: adds "clear" (x) button
                        });
                    } else if (padiToDataResponse.status == 500) {
                        Toast.fire({
                            icon: "error",
                            title: padiToDataResponse.message
                        });
                    } else {
                        $('#filter_paid_to').val('');
                        $('#filter_paid_to').select2({
            ...
            // fetch & show expense data in table
            function loaddata() {
                loadershow();
                ...
                table = $('#data').DataTable({
                    ...
                    ajax: {
                        ...
                        data: function(d) {
                            ...
                            d.filter_paid_to = $('#filter_paid_to').val();
