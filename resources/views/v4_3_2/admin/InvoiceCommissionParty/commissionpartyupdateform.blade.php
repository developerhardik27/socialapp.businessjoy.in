...
@section('page_title')
    {{ config('app.name') }} - Update Commission Party
@endsection
@section('title')
    Update Commission Party
@endsection


@section('form-content')
    <form id="commissionpartyupdateform">
        @csrf
...
@push('ajax')
    <script>
        $('document').ready(function() {
            ...
            // get selected commission party data and show it into fields

            // commission party update form  -> dynamic required attribute (if enter company name then only company name required otherwise only firstname)
            ...
            // get & set commission party old data in the form input
            var edit_id = @json($edit_id);
            let commissionPartySearchUrl = "{{ route('invoicecommissionparty.search', '__editId__') }}".replace('__editId__', edit_id);
            $.ajax({
                type: 'GET',
                url: commissionPartySearchUrl,
                data: {
                    token: "{{ session()->get('api_token') }}",
                ...
                success: function(response) {
                    if (response.status == 200) {
                        data = response.invoicecommissionparty; 
            ...
            $('#cancelbtn').on('click', function() {
                loadershow();
                window.location.href = "{{ route('admin.invoicecommissionparty') }}";
            });

            // subimt form
            $('#commissionpartyupdateform').submit(function(event) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ route('invoicecommissionparty.update', $edit_id) }}",
                    data: formdata,
                    success: function(response) {
                        ...
                        if (response.status == 200) {
                            ...
                            window.location.href =
                                "{{ route('admin.invoicecommissionparty') }}";
