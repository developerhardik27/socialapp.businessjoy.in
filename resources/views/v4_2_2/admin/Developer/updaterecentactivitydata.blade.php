...
@if (session('user_permissions.developermodule.recentactivitydata.add') == '1')
    ...
    @section('addnewbutton')
        <button data-toggle="tooltip" data-placement="bottom" data-original-title="Add New Data"
...
@section('form-content')
    <form id="recentactivityupdateform">
        @csrf
    ...
    <script>
        $('document').ready(function() {
            ...
            function loaddata(){
                ...
                let RecordSearchUrl = "{{ route('recentactivitydata.edit', '__recordId__') }}".replace('__recordId__', edit_id);
                 $.ajax({
                    type: 'GET',
                    url: RecordSearchUrl,
                    data: {
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            data = response.recentactivitydata
            ...
            $('#recentactivityupdateform').submit(function(event) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ route('recentactivitydata.update', $edit_id) }}",
                    data: formdata,
                    success: function(response) {
                        ...
                        if (response.status == 200) {
                            ...
                            window.location = "{{ route('admin.recentactivitydata') }}";
                        } else if (response.status == 500) {
                            Toast.fire({
                                icon: "error",
                                title: response.message
