        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="holidayModalLabel">Add Event</h5>
                ...
                <div class="modal-body">
                    <form id="holidayForm">
                        ...
                        <div class="form-group">
                            <label for="event_type">Event Type</label><span style="color: red">*</span>
                        ...
                        <div class="form-group" id="employee_field">
                            ...
                            <select class="form-control select2" name="emp_id[]" id="employee_name" multiple>
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadEmployees() {
                ...
                $.ajax({
                    url: "{{ route('employee.index') }}?" + getParams().toString(),
                    type: "GET",
                    success: function(response) {

                        let dropdown = $('#employee_name');
                        dropdown.empty();
                        ...
                        $.each(response.data, function(key, employee) {
                            ...
                            let name = employee.first_name + ' ' + employee.surname;

                            dropdown.append(`
                    <option value="${employee.id}" data-name="${name}">
                        ${name}
                    </option>
                `);
            ...
            $('#employee_name').on('change', function() {
                ...
                if (values.includes('all')) {
                    ...
                    $('#employee_name option').not('[value="all"]').prop('disabled', true);

                } else {

                    $('#employee_name option').prop('disabled', false);

                    $('#employee_name option:selected').each(function() {
                    ...
                    $('#emp_name').val(names.join(' And '));

                }

                if (values.length > 0 && !values.includes('all')) {
                    $('#employee_name option[value="all"]').prop('disabled', true);
                } else {
                    $('#employee_name option[value="all"]').prop('disabled', false);
                }
            ...
            function holidayCalendar() {
                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    events: function(fetchInfo, successCallback, failureCallback) {
                        fetch("{{ route('calendar.index') }}?" + getParams().toString())
                            ...
                            .then(data => {
                                ...
                                const events = data.holiday.map(item => {
                                    let title = item.event_type + item.event_title ||
                                        'No title';
                                    ...
                                    return {
                                        ...
                                        allDay: ['leave', 'increment', 'joining', 'tours']
                                            .includes(
                                                item.event_type),
                    ...
                    eventContent: function(arg) {
                        ...
                        let candidate_name = arg.event.extendedProps.candidate_name || '';
                        let employee_name = arg.event.extendedProps.employee_name || '';
                        ...
                        let empNameShort = employee_name ?
                            (employee_name.length > 20 ? employee_name.substring(0, 20) + '...' :
                                employee_name) :
                            '';

                        let candidateNameShort = candidate_name ?
                            (candidate_name.length > 20 ? candidate_name.substring(0, 20) + '...' :
                                candidate_name) :
                            '';

                        return {
                            html: `
                                <div style="line-height:1.3; color:${textColor};">

                                    <div style="font-weight:600; font-size:12px;">
                                        ${typeText}
                                    </div>

                                    ${title ? `<div style="border-top:1px solid rgba(0,0,0,0.25); margin:2px 0;"></div>` : ''}

                                    ${title ? `<div style="font-size:11px;"><span title="${title}">${title}</span></div>` : ''}

                                    ${empNameShort ? `<div style="border-top:1px solid rgba(0,0,0,0.25); margin:2px 0;"></div>` : ''}

                                    ${empNameShort ? `<div style="font-size:11px;"><span class="candidate-badge" title="${employee_name}">${empNameShort}</span></div>` : ''}

                                    ${candidateNameShort ? `<div style="border-top:1px solid rgba(0,0,0,0.25); margin:2px 0;"></div>` : ''}

                                    ${candidateNameShort ? `<div style="font-size:11px;"><span class="candidate-badge" title="${candidate_name}">${candidateNameShort}</span></div>` : ''}
            ...
            $('#holidayModal').on('hidden.bs.modal', function() {
                ...
                $("#holidayModalLabel").text('Add Event');
            ...
            $(document).on("click", ".edit-holiday", function() {
                ...
                const url = "{{ route('calendar.edit', ['id' => '__id__']) }}".replace('__id__', id);
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status !== 200) {
                            ...
                            Toast.fire({
                                icon: "error",
                                title: response.message || "Failed to fetch event"
                            });
                        ...
                        if (holiday.employee_id) {
                            ...
                            let empIds = typeof holiday.employee_id === "string" ?
                                JSON.parse(holiday.employee_id) :
                                holiday.employee_id;

                            $("#employee_name").val(empIds).trigger("change");

                            if (empIds.includes('all')) {

                                $('#employee_name option').not('[value="all"]').prop('disabled',
                                    true);
                                ...
                                $('#emp_name').val(holiday.employee_name);
            ...
            $(document).on("click", ".delete-holiday", function() {
                $("#holidayDetailModal").modal("hide");
                const id = $(this).data("id");
                const url = "{{ route('calendar.delete', ['id' => '__id__']) }}".replace('__id__', id);
                Swal.fire({
                    title: 'Are you sure?',
            ...
            $("#holidayForm").on("submit", function(e) {
                ...
                if (holidayId) {
                    url = "{{ route('calendar.update', ['id' => '__id__']) }}".replace('__id__',
                        holidayId);
                    type = 'PUT';
                }
                $.ajax({
