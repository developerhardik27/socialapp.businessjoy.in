...
@section('page_title')
    {{ config('app.name') }} - Edit letter variable setting
@endsection
@section('title')
    Edit letter variable setting
@endsection
...
@section('form-content')
    <form id="lettersettingupdateForm" class="border rounded p-4 mb-4">
        @csrf
...
@push('ajax')
    ...
    <script>
        $('document').ready(function() {
            ...
            function fetchdata(editid) {
                loadershow();
                let url = "{{ route('lettervariablesetting.edit', ['id' => '__id__']) }}".replace('__id__', editid);
                $.ajax({
                    url: url,
                    method: "get",
                    data: {
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            lettervariablesettings = response.data;
                            console.log(lettervariablesettings);
                            $("#variable_name").val(lettervariablesettings.variable.replace('$', ''));
                            $('#description').val(lettervariablesettings.description);
                            let fields = JSON.parse(lettervariablesettings.employee_fields);
                            ...
                            fields.forEach(function(item) {
                                ...
                                if (knownVariables.includes(item)) {
                                    $('#variable_formate_components').append(`
                                        <div class="col-sm-3 variable_formate_component mb-2">
                                            ...
                                            <select class="form-control variable_formate_component" data-type="variable">
                                                ...
                                                <option value="first_name" ${item === 'first_name' ? 'selected' : ''}>$fname</option>
                                                <option value="middle_name" ${item === 'middle_name' ? 'selected' : ''}>$mname</option>
                                                <option value="surname" ${item === 'surname' ? 'selected' : ''}>$sname</option>
                                                <option value="email" ${item === 'email' ? 'selected' : ''}>$email</option>
                                                <option value="mobile" ${item === 'mobile' ? 'selected' : ''}>$mobile</option>
                                                <option value="house_no_building_name" ${item === 'house_no_building_name' ? 'selected' : ''}>$house_no</option>
                                                <option value="road_name_area_colony" ${item === 'road_name_area_colony' ? 'selected' : ''}>$road_area_name</option>
                                                <option value="city_id" ${item === 'city_id' ? 'selected' : ''}>$city_name</option>
                                                <option value="state_id" ${item === 'state_id' ? 'selected' : ''}>$state_name</option>
                                                <option value="pincode" ${item === 'pincode' ? 'selected' : ''}>$pincode</option>
                                            </select>
                                            <span class="text-center">
                                                <i class="ri-delete-bin-line btn btn-primary iq-bg-danger remove_invoice_component"></i>
                                            </span>
                                        </div>
                                    `);
                                    ...
                                    <div class="col-sm-3 variable_formate_component mb-2">
                                        ...
                                        <input type="text"
                                            class="form-control variable_formate_component"
            ...
            $("#lettersettingupdateForm").on('submit', function(e) {
                e.preventDefault();
                loadershow();
                ...
                // Loop through dynamic components
                $('#variable_formate_components')
                    .children('.variable_formate_component')
                    .each(function() {

                        let $input = $(this).find('input[data-type="static_text"]');
                        let $select = $(this).find('select[data-type="variable"]');
                        ...
                        if ($input.length) {
                            ...
                            if (!val) {
                                hasError = true;
                                $(this).append(
                                    '<div class="error-msg " style="color:red;">Text field is required</div>'
                                    );
                            } else {
                                compArr.push(val);
                            }
                ...
                // Important for Laravel PUT
                formData.append('_method', 'PUT');

                // Append dynamic variables array
                compArr.forEach(function(value) {
                ...
                // AJAX Call
                $.ajax({
                    type: "post", // Laravel PUT via _method
                    url: "{{ route('lettervariablesetting.update', $edit_id) }}",
                    data: formData,
                    processData: false,
                    contentType: false,
                    headers: {
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 200) {
                            ...
                            window.location.href =
                                "{{ route('admin.letter_variable_setting') }}";
                        } else {
                            Toast.fire({
                                icon: "error",
                                title: response.message || "Something went wrong"
