...
@section('table-content')
    ...
    <div class="modal fade" id="subscriptionHistoryModal" tabindex="-1" role="dialog"
        aria-labelledby="subscriptionHistoryModalTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="subscriptionHistoryModalTitle">
                        <b>Subscription History - <span id="companyName"></span></b>
                    </h5>
...
@push('ajax')
    <script>
        $('document').ready(function() {
                    ...
                    columns: [{
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                if (data != 1) {
                                    actionBtns += `
                                        <span data-toggle="tooltip" data-placement="bottom" data-original-title="View Subscription History">
                                            <button type="button" data-subscription-history = '${data}' data-company-name = '${row.name}' class="subscription-history-btn btn btn-warning btn-rounded btn-sm my-0">
            ...
            $(document).on("click", ".subscription-history-btn", function() {
                loadershow();
                var companyId = $(this).data('subscription-history');
                var companyName = $(this).data('company-name');

                $('#companyName').text(companyName);
                $('#subscriptionHistoryContent').html(`
                ...
                // Fetch subscription history from billing_cycle_notes
                $.ajax({
                    type: 'GET',
                    url: "{{ route('company.subscription.history', '__companyId__') }}".replace(
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            displaySubscriptionHistory(response.data);
                        } else if (response.status == 404) {
                            $('#subscriptionHistoryContent').html(`
                                <div class="alert alert-info text-center">
                                    <i class="ri-information-line"></i>
                                    <strong>This company doesn't have any subscription records yet.</strong>
                                </div>
                            `);
                        } else {
                            $('#subscriptionHistoryContent').html(`
                                <div class="alert alert-danger text-center">
                                    ...
                                    <strong>${response.message || 'Something went wrong!'}</strong>
                                </div>
                            `);
                        }
                        loaderhide();
                    ...
                    error: function(xhr) {
                        ...
                        $('#subscriptionHistoryContent').html(`
                            <div class="alert alert-danger text-center">
                                ...
                                <p class="mb-0 mt-2">Failed to load subscription history. Please try again.</p>
                            </div>
                        `);
                    }
                });
            ...
            function displaySubscriptionHistory(historyData) {
                if (!historyData || historyData.length === 0) {
                    $('#subscriptionHistoryContent').html(`
                        <div class="alert alert-info text-center">
                            <i class="ri-information-line"></i>
                            <strong>No subscription history found</strong>
                            <p class="mb-0 mt-2">This company doesn't have any subscription records yet.</p>
                        </div>
                    `);
                    return;
                }
                ...
                historyData.forEach(function(record) {
                    const actionDate = record.created_at ?
                        new Date(record.created_at).toLocaleDateString() : 'N/A';
                    const paymentStatus = record.payment_status == 'paid' ? 'paid' : 'pending';
                    const billingCycle = record.billing_cycle_start_date && record.billing_cycle_end_date ?
                        `${new Date(record.billing_cycle_start_date).toLocaleDateString()} - ${new Date(record.billing_cycle_end_date).toLocaleDateString()}` :
                        'N/A';
                    const nextBilling = record.next_billing_date ?
                        new Date(record.next_billing_date).toLocaleDateString() : 'N/A';
                ...
                $('#subscriptionHistoryContent').html(historyHtml);
            }
