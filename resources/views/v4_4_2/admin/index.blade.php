                                    ...
                                    <div class="row">
                                            ...
                                            <div
                                                ...
                                                <div class="iq-card-header d-flex justify-content-between">
                                                    <div class="iq-header-title">
                                                        <h4 class="card-title">Company Users Overview</h4>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="col-md-12">
                                            <div
                                                ...
                                                <div class="iq-card-body">
                                                    <div id="subscription-pie-chart"></div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <script>
        $('document').ready(function() {
            ...
            function subscriptionPieChart() {
                const chartContainer = $("#subscription-pie-chart");
                ...
                fetch("{{ route('admindashboard') }}?" + params.toString())
                    .then(res => res.json())
                    .then(response => {
                        const apiData = response.admindashboard.subscriptionData;

                        if (!Array.isArray(apiData) || apiData.length === 0) {
                            chartContainer.html(
                                `<div style="text-align: center; padding: 2rem; font-weight: bold; color: #888;">No subscription data found</div>`
                            );
                            return;
                        }
                        ...
                        apiData.forEach((item, index) => {
                            item.color = statusColors[item.name] || colorPalette[index % colorPalette.length];
                        });
                        ...
                        const chart = new ApexCharts(document.querySelector("#subscription-pie-chart"), options);
                        chart.render();
                    })
                    .catch(error => {
                        console.error("Subscription chart data fetch error:", error);
                        chartContainer.html(
                            `<div style="text-align: center; padding: 2rem; color: red;">Error loading subscription data</div>`
                        );
                    });
            }
            ...
            function fetchAdminDashboardData() {
                $.ajax({
                    url: "{{ route('admindashboard') }}", // Replace this with your Laravel backend endpoint
                    method: 'GET',
                    data: {
                        user_id: "{{ session()->get('user_id') }}",
                        token: "{{ session()->get('api_token') }}",
                    ...
                    success: function(adminData) {
                        ...
                        companyData.forEach(item => {
                            ...
                            totalUsers.push(parseInt(item.total_users));
                            loggedInUsers.push(parseInt(item.logged_in_users));
                            activeUsers.push(parseInt(item.active_users));
                        });
