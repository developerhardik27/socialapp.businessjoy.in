        ...
        <div class="form-group">
            <div class="form-row">
                ...
                <div class="col-sm-4 mb-3">
                    ...
                    <input type="hidden" name="quotation_id" id="quotation_id" class="form-control" value="" />
                ...
                <div class="col-sm-4 mb-3" id="bank_details_div">
...
@push('ajax')
    ...
    <script>
        $('document').ready(function () {
            ...
            let sgst, cgst, gst, currentcurrency, currentcurrencysymbol,bankdetails__invoice;

            let viewthirdpartycompanies = "{{ session('user_permissions.invoicemodule.thirdpartyinvoice.view') == '1' }}";
            ...
            ajaxRequest('GET', "{{ route('getoverduedays.index') }}", {
                ...
                if (response.status == 200 && response.overdueday != '') {
                    ...
                    third_party_invoice = othersettingdata['third_party_invoice'];
                    bankdetails__invoice = othersettingdata['bankdetails'];
                    ...
                    if(third_party_invoice == 0 || !viewthirdpartycompanies){
                        $('#company_div').hide();
                    }
                    if(bankdetails__invoice == 0){
            ...
            ajaxRequest('GET', "{{ route('invoice.columnname') }}", {
                ...
                if (response.status == 200 && response.columnname != '') {
                    ...
                    if(invoice_data)
                    {
                        populateInvoiceData(invoice_data);
                    }
                    managetooltip();
            ...
            function populateInvoiceData(invoice_data) {
                const invdetails = invoice_data.data.quotationdetails;
                const productdetails = invoice_data.data.productdetails;
                
                
                $('#quotation_id').val(invoice_data.data.quotationdetails.id);
                // ─── Customer, payment, currency, account ─────────────────────────
                $('#customer').val(invdetails.customer_id).trigger('change');
                customers(invdetails.customer_id); // Populate customer-related data
                $('#payment').val(invdetails.payment_type);
                $('#currency').val(invdetails.currency_id);
                if(bankdetails__invoice == 1){
                    $('#acc_details').val(invdetails.account_id);
                }

                // const third_party_invoice = invdetails.third_party_invoice;
                if (third_party_invoice == 0 || !viewthirdpartycompanies) {
                    $('#company_div').hide();
                    if (viewthirdpartycompanies) companies(); // load all companies
                } else {
                    if(invdetails.third_party_quotation && invdetails.company_id)
                    {
                        companies(invdetails.company_id); 
                    }
                    else
                    {
                        companies(); // load all companies
                    }
                }
                ...
                const sgst = parseFloat(gstsettings.sgst || 0);
                const cgst = parseFloat(gstsettings.cgst || 0);
                const gst = parseFloat(gstsettings.gst || 0);
                const totalgstpercentage = sgst + cgst;
                ...
                productdetails.forEach((value) => {
                    ...
                    let rowHtml = `
                        <tr class="iteam_row_${dynamicidcount}" data-inventory="${value.inventory_product_id || ''}">
                            ${allColumnData.map(columnData => {
                                const columnNameRaw = columnData.column_name.replace(/\s+/g, '_');
                                const columnName = columnNameRaw.toLowerCase();

                                const isHidden = columnData.is_hide === 1 ? 'd-none' : '';
                                const val = normalizedValue[columnName] ?? columnData.default_value ?? '';

                                if (columnData.column_type === 'time') {
                                    return `<td class="${isHidden}"><input type="time" name="${columnName}_${dynamicidcount}" value="${val}" id="${columnName}_${dynamicidcount}" data-oldproduct-id="${value.id}" class="form-control iteam_${columnName}"></td>`;
                                } else if (['number', 'percentage', 'decimal'].includes(columnData.column_type)) {
                                    return `<td class="${isHidden}"><input type="number" step="any" name="${columnName}_${dynamicidcount}" value="${val}" id="${columnName}_${dynamicidcount}" data-id="${dynamicidcount}" data-oldproduct-id="${value.id}" class="form-control iteam_${columnName} counttotal calculation" min=0></td>`;
                                } else if (columnData.column_type === 'longtext') {
                                    return `<td class="${isHidden}"><textarea name="${columnName}_${dynamicidcount}" id="${columnName}_${dynamicidcount}" data-oldproduct-id="${value.id}" class="form-control iteam_${columnName}" rows="1">${val}</textarea></td>`;
                                } else {
                                    return `<td class="${isHidden}"><input type="text" name="${columnName}_${dynamicidcount}" value="${val}" id="${columnName}_${dynamicidcount}" data-oldproduct-id="${value.id}" class="form-control iteam_${columnName}" placeholder="${columnData.column_name}"></td>`;
                                }
                            }).join('')}
                            <td>
                                <input type="number" step="any" value="${value.amount || ''}" data-id="${dynamicidcount}" class="form-control iteam_Amount changeprice calculation" id="Amount_${dynamicidcount}" data-oldproduct-id="${value.id}" placeholder="Amount" name="Amount_${dynamicidcount}" min=0 required>
                            </td>
                            <td>
                                <span class="table-up"><a href="#!" class="indigo-text"><i class="fa fa-long-arrow-up"></i></a></span>
                                <span class="table-down"><a href="#!" class="indigo-text"><i class="fa fa-long-arrow-down"></i></a></span>
                            </td>
                            <td>
                                <span class="remove-row" data-id="${dynamicidcount}" data-oldproduct-id="${value.id}">
                                    <button data-toggle="tooltip" title="Delete Row" type="button" class="btn iq-bg-danger btn-rounded btn-sm"><i class="ri-delete-bin-2-line"></i></button>
                                </span>
                            </td>
                        </tr>
                    `;

                    targetRow.append(rowHtml);
        ...
        // ─── Currency symbol ──────────────────────────────────────────────
            const currentcurrencysymbol = $('#currency option:selected').data('symbol') || '';
            ...
            ajaxRequest('GET', "{{ route('consignorcopy.index') }}", {
                ...
                if (response.status == 200 && response.data != '') {
                    ...
                    if (consignorcopycount < 1 || lrcolumnlinks.length < 1) {
                        $('#lrdiv').hide();
                    }
                } else {
                    // Toast.fire({
            ...
            ajaxRequest('GET', "{{ route('country.index') }}", {
                ...
                if (response.status == 200 && response.country != '') {
                    ...
                    $.each(response.country, function (key, value) {
                     ...
                     if (invoice_data) {
                        $('#currency').val(invoice_data.data.quotationdetails.currency_id);
            ...
            function customers(customerid = 0) {
              ...
              function companies(companyid = 0) { 
                console.log('Loading companies with companyid:', companyid);
                loadershow();
                $('#company').html(`
                   <option selected="" value='0'> Select Company</option>
                   <option value="add_company" > Add New company </option>
                ...
                ajaxRequest('GET', "{{ route('invoice.companylist') }}", { 
                    token: API_TOKEN,
                    company_id: COMPANY_ID,
                    user_id: USER_ID
                }).done(function(response) {
                    if (response.status == 200 && response.company != '') {
                        ...
                        $.each(response.company, function(key, value) {
                            ...
                            $('#company').append(`
                                <option value='${value.id}'>${companyDetails}</option>
                            `);
                        });
            ...
            if(viewthirdpartycompanies){
                companies();
            }
             // Load countries into company modal
            ajaxRequest('GET', "{{ route('country.index') }}", { token: API_TOKEN })
                .done(function(response) {
                    if (response.status == 200) {
                        $.each(response.country, function(key, value) {
                            $('#company_modal_country').append(
            ...
            function loadCompanyState(id = "{{ session('user')['country_id'] }}") {
                ...
                ajaxRequest('GET', url, { token: API_TOKEN })
                    .done(function(response) {
                        if (response.status == 200) {
                            $.each(response.state, function(key, value) {
                                $('#company_modal_state').append(
            ...
            // Country change → reload state + city
            $('#company_modal_country').on('change', function() {
                loadCompanyState($(this).val());
            });

            // State change → reload city
            $('#company_modal_state').on('change', function() {
                loadCompanyCity($(this).val());
            });
            ...
            $('#companyform').submit(function(event) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('invoice.company.store') }}", 
                    data: formdata,
                    processData: false,
                    contentType: false,
                    success: function(response) {
                        if (response.status == 200) {
                            $('#companyform')[0].reset();
                            $('#companyModalScrollable').modal('hide');
                            companies(response.company_detail_id); // refresh company dropdown
            ...
            // Open company modal when "Add New Company" selected
            $('#company').on('change', function() {
                var companyid = $(this).val();
                if (companyid == 'add_company') {
                    $('#companyModalScrollable').modal('show'); // 👈 opens its own modal
                }
                loaderhide();
            });
            customers();
                    ...
                    <tr class="iteam_row_${addname}" data-inventory="null">
                        ${allColumnData.map(columnData => {
                            ...
                            if (columnData.column_type === 'time') {
                                ...
                                return `
                                    <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                        <input type="number" step="any" name="${columnName}_${addname}" value="${columnData.default_value || ''}" id='${columnName}_${addname}' data-id = ${addname} class="form-control iteam_${columnName} counttotal calculation"  min=0>
                                    </td>
                                `;
                            } else if (columnData.column_type === 'longtext') {
                                return `
                                    <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                        <textarea name="${columnName}_${addname}" value="${columnData.default_value || ''}" id='${columnName}_${addname}' class="form-control iteam_${columnName} " rows="1"> ${columnData.default_value || ''}</textarea>
                                    </td>
                                `;
                            } else {
                                return `
                            ...
                            if (value.id == selectedlr) {
                                $('#add_new_div').append(
                                    `<tr class="iteam_row_${addname}" >
                                        ...
                                        if (matchingLink) {
                                            let columnKey = matchingLink.lr_column;
                                            let combinedValues = [];
                                            ...
                                            if (columnKey === 'from') {
                                                ...
                                                let val = value[columnKey];
                                                if (val !== undefined && val !== null && val !== '') {
                                                    combinedValues.push(val);
                                                }
                                            }

                                            lrColumnValue = combinedValues.join(' To ');
                                        }
            ...
            function collectInvoiceDetails() {
                return {
                    ...
                    bankdetails: bankdetails__invoice,
