    ...
    <script>
        ...
        $('document').ready(function() {
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                @if (session('user_permissions.quotationmodule.quotation.add') == '1')
                                    actionBtns += `
                                    <span data-toggle="tooltip" title="Duplicate Quotation" data-placement="top"> 
                                        <button type="button"
                                            class="duplicate-btn btn btn-info btn-rounded btn-sm my-0 mb-2"
                                            data-id="${data}">
                               ...
                               if(row.invoice_id) {
                                    @if (session('user_permissions.invoicemodule.invoice.view') == '1')
                                        ...
                                        actionBtns += `                                             
                                            <span data-toggle="tooltip" data-placement="top" data-original-title="Download Invoice Pdf">
                                                <a href=${generateInvoicePdfUrl} target='_blank'>
                                                    <button type="button" class="download-btn btn btn-info btn-rounded btn-sm my-0 mb-2"><i class="ri-file-download-line"></i> </button>
                                                </a>
                                            </span>
                                        `;
                                    @endif
                                    ...
                                    @if (session('user_permissions.invoicemodule.invoice.add') == '1')
                                        actionBtns += `
                                        <span data-toggle="tooltip" title="Create Invoice" data-placement="top"> 
                                            <button type="button"
                                                class="create-invoice-btn btn btn-info btn-rounded btn-sm my-0 mb-2"
                                                data-id="${data}">
                                ...
                                `;
                                    ...
                                    if (row.is_editable == 1) {
                                        if (row.third_party_quotation == 1) {
                                            @if (session('user_permissions.quotationmodule.thirdpartyquotation.view') == '1')
                                                {{-- Has third party permission → redirect to edit --}}
                                                let quotationEditUrlThird =
                                                    "{{ route('admin.editquotation', '__quotationId__') }}"
                                                    .replace('__quotationId__', data);
                                                actionBtns += `
                                                <span>
                                                    <a href=${quotationEditUrlThird}>
                                                        <button type="button" data-id='${data}' data-toggle="tooltip" 
                                                            data-placement="bottom" data-original-title="Edit Quotation" 
                                                            class="edit-btn btn btn-success btn-rounded btn-sm my-0 mb-2">
                                            ...
                                            `;
                                            @else
                                                {{-- No third party permission → show popup --}}
                                                actionBtns += `
                                                <span>
                                                    <button type="button" data-id='${data}' data-toggle="tooltip" 
                                                        data-placement="bottom" data-original-title="Edit Quotation" 
                                                        class="no-permission-quotation-btn btn btn-success btn-rounded btn-sm my-0 mb-2">
                                                        <i class="ri-edit-fill"></i>
                                                    </button>
                                                </span>
                                            `;
                                            ...
                                            {{-- Normal quotation → redirect to edit --}}
                                ...
                                @if (session('user_permissions.quotationmodule.quotation.delete') == '1')
                                    if (row.third_party_quotation == 1) {
                                        @if (session('user_permissions.quotationmodule.thirdpartyquotation.view') == '1')
                                            {{-- Has third party permission → show delete button --}}
                                            actionBtns += `
                                            <span>
                                                <button type="button" data-id='${data}' data-toggle="tooltip" 
                                                    data-placement="bottom" data-original-title="Delete Quotation" 
                                        ...
                                        @else
                                            ...
                                            <span>
                                                <button type="button" data-id='${data}' data-toggle="tooltip" 
                                                    data-placement="bottom" data-original-title="Delete Quotation" 
                                                    class="no-permission-delete-quotation-btn btn btn-danger btn-rounded btn-sm my-0 mb-2">
                                                    <i class="ri-delete-bin-fill"></i>
                                                </button>
                                            </span>
                                        `;
                                        ...
                                        {{-- Normal quotation → show delete button --}}
                                        actionBtns += `
                                        <span>
                                            <button type="button" data-id='${data}' data-toggle="tooltip" 
                                                data-placement="bottom" data-original-title="Delete Quotation" 
            ...
            $(document).on('click', '.duplicate-btn', function() {
                let id = $(this).data('id'); // ✅ id is captured here

                var form = $('<form>', {
                    method: 'POST',
                    action: "{{ route('admin.quotation.duplicate') }}"
                });
            ...
            $(document).on('click', '.create-invoice-btn', function() {
                ...
                var form = $('<form>', {
                    method: 'POST',
                    action: "{{ route('admin.quotation.create-invoice') }}"
                });
            ...
            $(document).on('click', '.no-permission-quotation-btn', function() {
                Swal.fire({
                    icon: 'warning',
                    title: 'Permission Denied',
                    text: 'You do not have third party company permission. Please set all permissions so that you can edit this quotation.',
                    confirmButtonText: 'OK'
                });
            });
            $(document).on('click', '.no-permission-delete-quotation-btn', function() {
                Swal.fire({
                    icon: 'warning',
                    title: 'Permission Denied',
                    text: 'You do not have third party company permission. Please set all permissions so that you can delete this quotation.',
                    confirmButtonText: 'OK'
                });
            });
