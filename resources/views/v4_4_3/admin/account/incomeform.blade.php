...
@section('page_title')
    {{ config('app.name') }} - Add New Income
@endsection
@section('title')
    New Income
@endsection
...
@section('form-content')
    <form id="incomeform" name="incomeform" enctype="multipart/form-data">
        @csrf
            ...
            <div class="form-row">
                ...
                <div class="col-sm-6 mb-2">
                    ...
                    <input type="text" name="type" class="form-control" id="type"
                        placeholder="e.x., Tshirt, Shoes etc" />
...
@push('ajax')
    <script>
        $('document').ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        if (selectedId) {
                            ...
                            $('#income_subcategory').html(`<option value="">Select category</option>`)
                                .select2({
                                    placeholder: 'Select category',
                                    allowClear: true
                                });
            ...
            // ── Add category Modal Cancel button ────────────────────────────────────────────────────────────
            $('#category_cancelBtn').on('click', function() {
                $('#categoryform')[0].reset();
                $('#income_category').val('').trigger('change'); // ✅ correct ID + select2 reset
                $('.category-error-msg').text('');
                $('#addCategoryModal').modal('hide');
            });
            // ── Add Sub category Modal Cancel button ────────────────────────────────────────────────────────────
            $('#subcategory_cancelBtn').on('click', function() {
                $('#subcategoryform')[0].reset();
                $('#income_subcategory').val('').trigger('change');
            ...
            // Add  paid by (customer) modal cancel btn
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 200) {
                            ...
                            window.location = "{{ route('admin.income') }}";
