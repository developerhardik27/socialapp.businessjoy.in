...
@section('page_title')
    {{ config('app.name') }} - Update Expense Details
@endsection
@section('title')
...
@section('form-content')
    <form id="expenseupdateform" name="expenseupdateform" enctype="multipart/form-data">
        @csrf
        <div class="form-group">
            <div class="form-row">
                ...
                <div class="col-sm-6 mb-2">
                    <label for="expense_subcategory">Sub Category</label>
                    <select id="expense_subcategory" name="expense_subcategory" class="form-control">
                        <option value="">Select Sub Category</option>
                    </select>
                    <span class="error-msg" id="error-subcategory" style="color: red"></span>
                </div>
                ...
                <div class="col-sm-12">
                    ...
                    <button type="reset" data-toggle="tooltip" data-placement="bottom"
                        data-original-title="Reset Expense Details"
                        class="btn iq-bg-danger float-right mr-2">Reset</button>
                    <button type="submit" data-toggle="tooltip" data-placement="bottom"
                        data-original-title="Update Expense Details"
                        class="btn btn-primary float-right my-0">Save</button>
                </div>
            </div>
        </div>
    </form>
    {{-- paid to add new (customer) modal --}}
    <div class="modal fade" id="exampleModalScrollable" tabindex="-1" role="dialog"
        aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalScrollableTitle">Add New Paid To</h5>
...
@push('ajax')
    <script>
        $('document').ready(function() {
            ...
            let expenseSearchUrl = "{{ route('expense.edit', '__editId__') }}".replace('__editId__', edit_id);
            $.ajax({
                type: 'GET',
                url: expenseSearchUrl,
                data: {
            ...
            //load categorys
            function loadCategory(selectedId = '', selectedSubId = '') { // ← add param
            ...
            function loadsubCategory(categoryId, selectedSubId = '') {
                const $select = $('#expense_subcategory');
                $select.html(
            ...
            $('#expense_subcategory').on('change', function() {
                const val = $(this).val();
                if (val == 'add_new') {
                    const parentCategoryId = $('#expense_category').val();
            ...
            // ── category add modalCancel button ────────────────────────────────────────────────────────────
            ...
            // ── Sub category add modalCancel button ────────────────────────────────────────────────────────────
            $('#subcategory_cancelBtn').on('click', function() {
                $('#subcategoryform')[0].reset();
                $('#expense_subcategory').val(subcategory_id).trigger(
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            $('#categoryform')[0].reset();
                            $('#addCategoryModal').modal('hide');
                            loadCategory(response.category_id); // ✅ auto-select new category
                            Toast.fire({
                                icon: 'success',
                                title: response.message
                            });
            ...
            $('#subcategoryform').submit(function(event) {
                ...
                $('#sub_category_parent_id').val($('#expense_category').val());
            ...
            //loadpaidto (customer)
            function loadPaidTo(selectedId = '') {
                let customer_id = selectedId;
                loadershow();
                $('#paid_to').html(`
                    <option value="">Select Paid To</option>
                    <option value="add_new">+ Add New Paid To</option>
                `);

                ajaxRequest('GET', "{{ route('customer.accountexpensecustomer') }}", {
                    token: API_TOKEN,
                    company_id: COMPANY_ID,
                    user_id: USER_ID,
                    ...
                    if (response.status == 200 && response.data != '') {
                        ...
                        $('#paid_to').append(`<option disabled>No Paid To Found</option>`);
                    }
                    if ($('#paid_to').data('select2')) {
                        $('#paid_to').select2('destroy');
                    }
                    $('#paid_to').val(selectedId).select2({
                        placeholder: 'Select Paid To',
                        allowClear: true,
                    });
                    if (selectedId) {
                        const selectedOption = $('#paid_to option[value="' + selectedId + '"]');
                        $('#customer_id').val(selectedId);
                        $('input[name="paid_to"]').val(selectedOption.data('name') || '');
                    }
                    loaderhide();
                }).fail(function(xhr) {
                    loaderhide();
            ...
            // When paid_to changes,
            $('#paid_to').on('change', function() {
                var payid = $(this).val();
                ...
                if (payid == 'add_new') {
                    ...
                    $('#paid_to_name').val(selectedOption.data('name') || '');
                }
                loaderhide();
            });
            //paid to (customer) modal cancel btn
            $('#modal_cancelBtn').on('click', function() {
                $('#customerform')[0].reset();
                $('#exampleModalScrollable').modal('hide');
                $('#customer option:first').prop('selected', true);
            })
            //paid to (customer) add form 
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 200) {
                            ...
                            loadPaidTo(response.customer_id);
                            Toast.fire({
                                icon: "success",
                                title: response.message
                            });
            ...
            // redirect on expense list page on click cancel btn
            ...
            // subimt form
            $('#expenseupdateform').submit(function(event) {
                event.preventDefault();
                loadershow();
                $('.error-msg').text('');
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('expense.update', $edit_id) }}",
                    data: formdata,
                    processData: false,
                    contentType: false,
                    success: function(response) {
