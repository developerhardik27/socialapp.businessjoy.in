...
class customerController extends commonController
{
    ...
    public $userId, $companyId, $masterdbname, $rp, $customerModel, $invoice_other_settingModel, $quotation_other_settingModel, $account_other_settingModel, $incomeModal,$expenseModal;
    ...
    public function __construct(Request $request)
    {
        ...
        $this->incomeModal = $this->getmodel('Income');
        $this->expenseModal = $this->getmodel('Expense');
    }
    ...
    public function invoicecustomer(Request $request)
    {
        ...
        $baseQuery = function () use ($columns) {
            return $this->customerModel::leftJoin($this->masterdbname . '.country', 'customers.country_id', '=', $this->masterdbname . '.country.id')
        ...
        // ── Query 1 : customers matching the dropdown types ──────────────────────
        $dropdownQuery = $baseQuery()->whereIn('customers.customer_type', $dropdown);

        if ($this->rp['invoicemodule']['customer']['alldata'] != 1) {
            $dropdownQuery->where('customers.created_by', $this->userId);
        }

        $customers = $dropdownQuery->get();

        // ── Query 2 : specific customer from request (if provided) ───────────────
        if (!empty($request->customer_id)) {
            $specificCustomer = $baseQuery()
                ->where('customers.id', $request->customer_id)
                ->get();

            // Merge and remove duplicates by 'id'
            $customers = $customers
                ->merge($specificCustomer)
                ->unique('id')
                ->values();
        }
    ...
    public function accountincomecustomer(Request $request)
    {
        if ($this->rp['accountmodule']['accountcustomer']['view'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }

        $dropdown = $this->account_other_settingModel::where('id', 1)->value('income_customer_dropdown');
        $dropdown = json_decode($dropdown, true);
        ...
        if ($this->rp['accountmodule']['accountcustomer']['alldata'] != 1) {
            $dropdownQuery->where('customers.created_by', $this->userId);
        }
        ...
        if (!empty($request->customer_id) && $request->customer_id != 'all') {
            ...
            $income_customers = $this->incomeModal
                ::where('is_deleted', 0)
                ->pluck('customer_id');

            // 'all' passed → fetch ALL active customers regardless of dropdown
            $allCustomers = $baseQuery()->whereIn('customers.id', $income_customers)->get();
    ...
    public function accountexpensecustomer(Request $request)
    {
        if ($this->rp['accountmodule']['accountcustomer']['view'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }

        $dropdown = $this->account_other_settingModel::where('id', 1)->value('expense_customer_dropdown');
        $dropdown = json_decode($dropdown, true);
        ...
        if (!empty($request->customer_id) && $request->customer_id != 'all') {
            ...
            $income_customers = $this->expenseModal
                ::where('is_deleted', 0)
                ->pluck('customer_id');
    ...
    public function quotationcustomer(Request $request)
    {
        ...
        $dropdown = $this->quotation_other_settingModel::where('id', 1)->value('customer_dropdown');
        $dropdown = json_decode($dropdown, true);
    ...
    public function datatable(Request $request)
    {
        ...
        if ($request->customer_type == 'invoice') {
            $dropdown = $this->invoice_other_settingModel::where('id', 1)->value('customer_dropdown');
        } else if ($request->customer_type == 'quotation') {
            $dropdown = $this->quotation_other_settingModel::where('id', 1)->value('customer_dropdown');
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($customerType == 'invoice') {
                $customerid = $this->invoice_other_settingModel::find(1);
            } else {
                $customerid = $this->quotation_other_settingModel::find(1);
            }

            $customer = $this->customerModel::create([ //insert customer record and return customer id
                'customer_id' => $customerType == 'invoice' ? $customerid->current_customer_id : null,
                'quotation_customer_id' => $customerType == 'quotation' ? $customerid->current_customer_id : null,
            ...
            if ($customer) {
                $customerid->current_customer_id += 1; // update customer id in invoice/quotation other setting table
                $customerid->save();
