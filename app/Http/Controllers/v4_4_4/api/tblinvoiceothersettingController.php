...
class tblinvoiceothersettingController extends commonController
{
    ...
    {
        $commissionsettings = $this->invoice_other_settingModel::where('is_deleted', 0)->value('commission');

        if (!$commissionsettings) {
            return $this->successresponse(404, 'commissionsettings', 'No Records Found');
        }

        return $this->successresponse(200, 'commissionsettings', $commissionsettings);
    }
    ...
    public function overduedayupdate(Request $request, string $id)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'third_party_invoice' => 'required|numeric',
            'bankdetails' => 'required|numeric',
        ]);
        ...
        if ($validator->fails()) {
            ...
            $overdueday->update([
                ...
                'third_party_invoice' => $request->third_party_invoice,
    ...
    public function commissionsettingsupdate(Request $request, string $id)
    {
        //condition for check if user has permission to search  record
        if ($this->rp['invoicemodule']['invoicecommissionsetting']['edit'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
        ...
        if ($validator->fails()) {
            ...
            $commissionsettings = $this->invoice_other_settingModel::find($id);

            if (!$commissionsettings) {
                return $this->successresponse(404, 'message', 'No such commission settings found!');
            }
    ...
    {
        $validator = Validator::make($request->all(), [
            'customer_dropdown' => 'required|array',
            'invoice_commission_party_dropdown' => 'required|array',
            'customer_dropdown.*' => 'string',
            'invoice_commission_party_dropdown.*' => 'string',
            'user_id' => 'required|numeric',
        ]);
        ...
        // Permission check
        if ($this->rp['invoicemodule']['invoiceformsetting']['edit'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
        ...
        $getsettings->update([
            // store as JSON
            'customer_dropdown' => json_encode($request->customer_dropdown),
            'commission_party_dropdown' => json_encode($request->invoice_commission_party_dropdown),
            'updated_by' => $this->userId,
            'updated_at' => date('Y-m-d H:i:s'),
        ]);
