...
class invoiceController extends commonController
{
    ...
    public function store(Request $request)
    {
        ...
        return $this->executeTransaction(function () use ($request) {
            ...
            if ($validator->fails()) {
                ...
                if ($request->bankdetails == 1) {
                    if ($request->bank_account == null) {
                        return $this->errorresponse(422, ["bank_account" => ['Bank account details are required when bank details option is active!']]);
                    }
                }
                ...
                if ($company_details) {
                    ...
                    $invoicerec['bankdetails'] = $data['bankdetails'];
                    // dd($invoicerec);
                    ...
                    if ($invoice) {
                        ...
                        if (isset($data['quotation_id']) && $data['quotation_id'] != '') {
                            DB::connection('dynamic_connection')->table('quotations')->where('id', $data['quotation_id'])->update([
                                'invoice_id' => $inv_id
                            ]);
                        }
    ...
    public function destroy(string $id)
    {
        return $this->executeTransaction(function () use ($id) {
            ...
            if ($invoices) {
                $quotation = DB::connection('dynamic_connection')->table('quotations')
                    ->where('invoice_id', $id)
