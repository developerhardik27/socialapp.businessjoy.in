...
class consignorController extends commonController
{
    ...
    {
        $dropdown = $this->logistic_settingsModel::where('id', 1)->value('consignor_dropdown');
        $dropdown = json_decode($dropdown, true);

        $customers = $this->consignorModel::where('customers.is_deleted', 0)
            ->where('customers.customer_type', 'consignor')
    ...
    public function index(Request $request)
    {
        ...
        $totalcount = $customers->count(); // count total record
