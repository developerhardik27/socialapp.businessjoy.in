...
class PaymentController extends commonController
{
    ...
    public $userId, $companyId, $masterdbname, $invoiceModel, $payment_detailsModel, $rp, $incomeModel, $ledgerModel, $expenseModel;
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($payment_inserted) {
                ...
                $description = "Invoice:$invoice->inv_no, Payment Received <br> $request->description";

                $income = [
                    'income_id' => $invoice->id,
                    'income_details_id' =>  $payment_inserted->id,
                    'reference_no' => $receipt_number,
                    'description' => $description,
                    'amount' => $paid_amount + $tds_paid_amount,
                ...
                $ledger = [
                    ...
                    'description' => $description,
                    'reference_no' => $receipt_number,
                    'credited' => $paid_amount + $tds_paid_amount,
                    'type' => 'income',
    ...
    public function destroy($id)
    {
        return $this->executeTransaction(function () use ($id) {
            ...
            $Income = $this->incomeModel::where('income_details_id', $id)->first();
            ...
            if ($Income) {
                ...
                $updateLedger = $this->ledgerModel::where('payment_id', $Income->id)
                    ->where('type', 'income')
                    ->update([
                        'is_deleted' => 1
                    ]);
                ...
                if (!$updateLedger) {
                    throw new \Exception("Failed to delete payment details");
                }
            }
