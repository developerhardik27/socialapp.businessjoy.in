...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $jsoninvoiceothersettingdetails = app($invoiceothersettingController)->getcommissionsettings($company_id);
        ...
        if ($invoiceothersettingdetails->status == 200 && isset($invoiceothersettingdetails->commissionsettings)) {
            $commission = $invoiceothersettingdetails->commissionsettings;
        }

        return view($this->version . '.admin.Invoice.invoice', compact('search', 'commission'));
    }
    ...
    public function create()
    {
        ...
        if ($invoiceothersettingdetails->status != 200 || count($invoiceothersettingdetails->pattern) < 2) {
            return view($this->version . '.admin.Invoice.othersettings', ['user_id' => $user_id, 'company_id' => $company_id, 'message' => 'yes']);
        }
        ...
        if ($bdetails->status != 200) {
            return view($this->version . '.admin.Bank.bankform', ['user_id' => $user_id, 'company_id' => $company_id, 'message' => 'yes']);
        }
        ...
        if ($columndetails->status != 200) {
            return view($this->version . '.admin.Invoice.managecolumn', ['user_id' => $user_id, 'company_id' => $company_id, 'message' => 'yes']);
        }

        // return view($this->version . '.admin.Invoice.invoiceform', ['user_id' => $user_id, 'company_id' => $company_id]);
        // dd(session('type'));
        if (session('type') === 'invoice_creation') {
            $invoice_data = session()->pull('invoice_data'); // Get and remove from session

            if (!$invoice_data) {
                // Duplicate data is missing → redirect back with message
                session(['type' => 'normal']);
                return redirect()->route('admin.quotation')->with('message', 'Invoice data is not available or has expired.');
            }

            // Duplicate data exists → show form with data
            session(['type' => 'invoice_creation']); // set type for view logic

            return view($this->version . '.admin.Invoice.invoiceform', [
                'user_id'       => $user_id,
                'company_id'    => $company_id,
