...
class logisticothersettingsController extends commonController
{
    ...
    public function consignmentnotenumberstore(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (!$logisticsetting) {
                return $this->successresponse(404, 'message', 'No such lr number setting found!');
            }
            ...
            if ($latestCopy) {
                if ($logisticsetting->current_consignment_note_no > $request->consignment_note_number) {
                    return $this->successresponse(500, 'message', 'Please enter lr number higher than or equal to ' . $logisticsetting->current_consignment_note_no);
                }
            }
            ...
            return $this->successresponse(200, 'message', 'lr number settings succesfully updated');
        }
    }
    ...
    {
        $validator = Validator::make($request->all(), [
            'consignee_dropdown' => 'required|array',
            'consignor_dropdown' => 'required|array',
            'consignee_dropdown.*' => 'string',
            'consignor_dropdown.*' => 'string',
            'user_id' => 'required|numeric',
        ]);
        ...
        $getsettings->update([
            // store as JSON
            'consignee_dropdown' => json_encode($request->consignee_dropdown),
            'consignor_dropdown' => json_encode($request->consignor_dropdown),
            'updated_by' => $this->userId,
            'updated_at' => date('Y-m-d H:i:s'),
        ]);

        return $this->successresponse(200, 'message', 'Settings successfully updated');
    }
