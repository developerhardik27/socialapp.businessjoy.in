...
class tblquotationothersettingController extends commonController
{
    ...
    public function overduedayupdate(Request $request, string $id)
    {
        $validator = Validator::make($request->all(), [
            ...
            'third_party_quotation' => 'nullable|numeric',
        ...
        if ($validator->fails()) {
            ...
            if (!$overdueday) { 
                return $this->successresponse(404, 'message', 'No such settings found!');
            }
            date_default_timezone_set('Asia/Kolkata');
            $overdueday->update([
                'overdue_day' => $request->expired_day,
                'year_start' => $request->year_start_date,
                'third_party_quotation' => $request->third_party_quotation ?? 0,
    ...
    public function customerdropdown(Request $request)
    {
        ...
        // Permission check
        if ($this->rp['quotationmodule']['quotationformsetting']['edit'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
