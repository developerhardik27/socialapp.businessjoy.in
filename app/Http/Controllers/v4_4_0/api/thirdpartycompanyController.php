...
{
    public $userId, $companyId, $rp, $quotation_other_settingModel, $masterdbname, $third_party_companyModel, $third_party_companies_invoiceModel, $invoice_other_settingsModel;
    ...
    public function __construct(Request $request)
    {
        ...
        $this->third_party_companies_invoiceModel = $this->getmodel('third_party_companies_invoice');
        $this->quotation_other_settingModel = $this->getmodel('quotation_other_setting');
        $this->invoice_other_settingsModel = $this->getmodel('invoice_other_settings');
    }

    // for using pdf 
    public function quotationcompanycompanydetailspdf($id)
    {
        $companydetails = $this->third_party_companyModel::leftJoin($this->masterdbname . '.country', 'third_party_companies.country_id', '=', $this->masterdbname . '.country.id')
            ->leftJoin($this->masterdbname . '.state', 'third_party_companies.state_id', '=', $this->masterdbname . '.state.id')
            ->leftJoin($this->masterdbname . '.city', 'third_party_companies.city_id', '=', $this->masterdbname . '.city.id')
            ->select(
    ...
    public function quotationcompanycompanylist(Request $request)
    {
     ...
     */
    public function quotationcompanyindex(Request $request)
    {
        if ($this->rp['quotationmodule']['thirdpartyquotation']['view'] != 1) {
    ...
    public function quotationcompanystore(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            return $this->executeTransaction(function () use ($request) {
                ...
                if ($company_details) {
                    return $this->successresponse(200, 'message', 'Company succesfully added', 'company_detail_id', $company_details->id);
                } else {
     ...
     */
    public function quotationcompanyshow(string $id)
    {
        if ($this->rp['quotationmodule']['thirdpartyquotation']['view'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
     ...
     */
    public function quotationcompanyupdate(Request $request, string $id)
    {
        if ($this->rp['quotationmodule']['thirdpartyquotation']['edit'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
     ...
     */
    public function quotationcompanydestroy(string $id)
    {
    ...
    // company  
    public function quotationcompanystatusupdate(Request $request, string $id)
    {
        if ($this->rp['quotationmodule']['thirdpartyquotation']['edit'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
    ...
    public function invoicecompanycompanydetailspdf($id)
    {
        $companydetails = $this->third_party_companyModel::leftJoin($this->masterdbname . '.country', 'third_party_companies.country_id', '=', $this->masterdbname . '.country.id')
            ->leftJoin($this->masterdbname . '.state', 'third_party_companies.state_id', '=', $this->masterdbname . '.state.id')
            ->leftJoin($this->masterdbname . '.city', 'third_party_companies.city_id', '=', $this->masterdbname . '.city.id')
            ...
            ->where('third_party_companies_invoices.id', $id)->get();
    ...
    public function invoicecompanycompanylist(Request $request)
    {
        if ($this->rp['invoicemodule']['thirdpartyinvoice']['view'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }

        $company = $this->third_party_companyModel::where('is_deleted', 0);

        if ($this->rp['invoicemodule']['thirdpartyinvoice']['alldata'] != 1) {
            $company->where('created_by', $this->userId);
        }
     ...
     * Display a listing of the resource.
     */
    public function invoicecompanyindex(Request $request)
    {
        if ($this->rp['invoicemodule']['thirdpartyinvoice']['view'] != 1) {
            return response()->json([
                'status' => 500,
        ...
        $company = $this->third_party_companyModel::join($this->masterdbname . '.country', 'third_party_companies.country_id', '=', $this->masterdbname . '.country.id')
            ...
            ->select(
                'third_party_companies.id',
                'third_party_companies.name',
                'third_party_companies_invoices.email', 
                DB::raw("CAST(third_party_companies.contact_no AS CHAR) as contact_no"),
                DB::raw("CAST(third_party_companies.alternative_number AS CHAR) as alternative_number"),
                'third_party_companies.house_no_building_name',
                'third_party_companies.road_name_area_colony',
    ...
    public function invoicecompanystore(Request $request)
    {
        if ($this->rp['invoicemodule']['thirdpartyinvoice']['add'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
        ...
        if ($validator->fails()) {
            ...
            return $this->executeTransaction(function () use ($request) {
                ...
                if (($request->hasFile('img') && $request->file('img') != null)) {
                    ...
                    $dirPath = public_path('uploads/') . $this->companyId . '/thirdpartyinvoice';
                    ...
                    if ($image) {
                        $imageName = $request->name . time() . '.' . $image->getClientOriginalExtension();
                        $image->move($dirPath, $imageName); // upload image
                        $imageName = $this->companyId . '/thirdpartyinvoice/' . $imageName;
                    }
                }
     ...
     * Display the specified resource.
     */
    public function invoicecompanyshow(string $id)
    {
        if ($this->rp['invoicemodule']['thirdpartyinvoice']['view'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
     ...
     * Show the form for editing the specified resource.
     */
    public function invoicecompanyedit(string $id)
    {
     ...
     * Update the specified resource in storage.
     */
    public function invoicecompanyupdate(Request $request, string $id)
    {
        if ($this->rp['invoicemodule']['thirdpartyinvoice']['edit'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
        ...
        return $this->executeTransaction(function () use ($request, $id) {
            ...
            if ($validator->fails()) {
                ...
                if (($request->hasFile('img') && $request->file('img') != null)) {
                    ...
                    $dirPath = public_path('uploads/') . $id . '/thirdpartyinvoice';

                    // Check if image file is uploaded
                    if ($image) {
                        $imageName = $request->name . time() . '.' . $image->getClientOriginalExtension();
                        $image->move($dirPath, $imageName); // upload image
                        $imageName = $id . '/thirdpartyinvoice/' . $imageName;
                    }
                }
     ...
     * Remove the specified resource from storage.
     */
    public function invoicecompanydestroy(string $id)
    {

        return $this->executeTransaction(function () use ($id) {

            if ($this->rp['invoicemodule']['thirdpartyinvoice']['delete'] != 1) {
                return $this->successresponse(500, 'message', 'You are Unauthorized');
            }
    ...
    // company  
    public function invoicecompanystatusupdate(Request $request, string $id)
    {
        if ($this->rp['invoicemodule']['thirdpartyinvoice']['edit'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }
